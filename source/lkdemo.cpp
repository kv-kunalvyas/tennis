...
int main( int argc, char** argv )
{
    ...
    Mat gray, prevGray, image, player;
    ...
    for(;;)
    {
        ...
        //Mat player;
        cap >> frame;
        //Mat cap = cap(Rect(160, 140, 320, 160));
        ...
        frame.copyTo(image);
        frame.copyTo(player);
        image = frame(Rect(160, 140, 320, 160));
        
        vector<Rect> found;
        hog.detectMultiScale(image, found, 0, Size(3,3), Size(1,1), 5, 1);
        
        for (size_t i = 0; i < found.size(); i++)
        {
            //Mat roi = frame(Rect(found[i]));
            //cvtColor(roi, gray, COLOR_BGR2GRAY);
            rectangle(image, found[i], CV_RGB(0, 255, 0), 1);
            player = image(found[i]);
            cout << found[i].width << endl;
            cout << found[i].height << endl;
        }
        cvtColor(player, gray, COLOR_BGR2GRAY);
        ...
        else if( !points[0].empty() )
        {
            ...
            for( i = k = 0; i < points[1].size(); i++ )
            {
                ...
                circle(player, points[1][i], 3, Scalar(0,255,0), -1, 8);
            }
            points[1].resize(k);
        }
