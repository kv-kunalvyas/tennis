function [nMM nST nCC Conf] = FlatMM_Tennis_10events(Vocabulary1,Vocabulary2,n,st)

Test_S=Vocabulary2;
[Test_T Test_Seq]=Build_master_topology_Tennis_10events(Test_S);
B = Test_T;
m=1;c=1;
...
for iter = st:length(Vocabulary1)
    ...
    for i=1:length(Vocabulary1)-n-iter+1
        ...
        [Train_T{i} Train_Seq]=Build_master_topology_Tennis_10events(Train_S);
        A = Train_T{i};
        %load Badminton_trained10.mat
        %A = Badminton_Trained10;
        Ts=zeros(10,10);
        k=0;
