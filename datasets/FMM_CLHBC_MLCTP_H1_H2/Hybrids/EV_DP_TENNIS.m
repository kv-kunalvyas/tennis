...
for mcm=228:length(idxOfUnique) %Topology_number:Topology_number %1:length(idxOfUnique) %920 %1:length(idxOfUnique) %1:length(part) %88:88 %%920 is the final number
...
T1MLDP_CLHBC_Confmatrix=mat2tmat(summ./100);
T1MLDP_CLHBC_EventsPrediction=sumnCC./100;
T1MLDP_CLHBC_mean=mean(me(Topology_number,:));
T1MLDP_CLHBC_std=std(me(Topology_number,:));

% save T1MLDP_CLHBC.mat T1MLDP_CLHBC_Confmatrix T1MLDP_CLHBC_EventsPrediction T1MLDP_CLHBC_mean T1MLDP_CLHBC_std
