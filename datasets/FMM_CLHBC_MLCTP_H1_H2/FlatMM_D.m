function [nMM nST nCC Conf] = FlatMM_D(Driving_Events,Train_Seqs,Test_Seqs,n,st)                
m=1;c=1;
Conf = zeros(10,10);
EV ={'S';'TSt';'TLe';'TRi';'LR';'LA';'LG';'SiLL';'SiRR';'Sp'};

for iter = st:length(Driving_Events)-n
    iter/(length(Driving_Events)-n)
l=1;
for i=1:length(Driving_Events)-n-iter+1
    
     Train_Seqst=Train_Seqs(i:i+iter-1);
     [Train_T{i} Train_Seq]=Build_master_topology_Driver(Train_Seqst);
     [Test_T{i} Test_Seq]=Build_master_topology_Driver(Test_Seqs);
     A = Train_T{i};
     B = Test_T{i};
     Ts=zeros(10,10);
     k=0;
